...
date: 2026-05-04
pdf: "https://arxiv.org/pdf/2605.02391v2"
tags: ["query:dml-dp"]
score: 6.0
evidence: 注入噪声以实现差分隐私
tldr: 本文针对流式运行时监控中敏感信息泄露的风险，提出了一种自动实施差分隐私的方法。由于时间算子会导致输入值在不同时间点多次影响输出，研究通过分析时间依赖性并在监控规范的战略位置注入校准噪声，结合树状机制减少精度损失。该方法在保护隐私的同时维持了监控输出的实用性，并在公共交通监控案例中验证了其有效性。
selection_source: fresh_fetch
motivation: 流式监控系统在处理隐私敏感数据时，时间算子会导致个人信息被重复披露，难以直接应用传统的差分隐私技术。
method: 通过分析规范中的时间依赖性，在战略位置注入校准噪声，并利用树状机制优化聚合算子的精度。
result: 开发出一套能自动在监控规范中强制执行差分隐私的框架，并在公共交通使用情况监控的案例研究中证明了其可行性。
conclusion: 该方法有效解决了运行时监控中的隐私保护难题，实现了隐私性与数据实用性之间的良好平衡。
---

## 摘要
现代基于流的监控器收集被观测系统运行时行为的详细统计数据。如果系统在隐私敏感的环境中运行，这会带来泄露敏感信息的风险。差分隐私是保护敏感信息的先进方法，然而，将其集成到运行时监控中具有挑战性：时间算子可能导致单个输入值随时间影响多个输出，从而导致隐私信息的重复泄露。我们提出了一种方法，通过分析时间依赖性并在规范中注入经过精心校准的噪声，在基于流的监控规范中自动强制执行差分隐私。为了保持输出的效用，我们在规范中识别战略性选择的噪声注入位置，并利用基于树的机制来减轻注入聚合算子的噪声所导致的准确性损失。我们通过一个关于公共交通使用监控的案例研究，证明了我们方法的实用性和有效性。

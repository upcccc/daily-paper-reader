...
tldr: 本文提出了一种支持空中联邦边缘学习（OTA-FEEL）的多目标集成感知与通信（ISAC）框架，利用双用途上行信号同时处理感知与学习任务。通过正交脉冲整形抑制干扰，并采用分布式似然最大化估计目标坐标，结合多目标优化算法平衡学习均方误差与感知误差界。研究证明该框架在不损害联邦学习性能的前提下，显著提升了协作感知精度并达到了克拉美-罗界。
selection_source: fresh_fetch
motivation: 旨在解决在联邦边缘学习中如何高效复用上行信号，以同时实现高精度协作感知与大规模数据聚合的挑战。
method: 提出正交脉冲整形抑制干扰，并利用分布式似然最大化与epsilon-约束法求解最小化学习MSE和感知CRB的多目标优化问题。
result: 数值结果表明，该框架在保证OTA-FEEL任务性能的同时显著增强了感知精度，且性能优于传统的单次协作感知方案。
conclusion: 该研究验证了双用途信号在ISAC系统中的高效性，为未来边缘智能网络中感知与通信的深度融合提供了理论支撑和优化方案。
---

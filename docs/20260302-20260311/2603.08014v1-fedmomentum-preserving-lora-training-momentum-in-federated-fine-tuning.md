...
tags: ["query:dml-dp"]
score: 8.0
evidence: 基于LoRA的隐私保护联邦微调
tldr: 针对联邦学习中 LoRA 微调存在的聚合噪声和训练动量丢失问题，本文提出 FedMomentum 框架。该方法通过 SVD 分解在数学正确的聚合基础上提取主成分以重构 LoRA 模块，并将残差合并至主干网络以保留语义信息。实验证明该方法在收敛速度和最终精度上均优于现有技术，有效提升了联邦微调的效率。
selection_source: fresh_fetch
motivation: 传统的 LoRA 联邦聚合方式存在数学上的不正确性，且现有改进方案会导致训练动量丢失，限制了模型的收敛速度和性能。
method: 提出 FedMomentum 框架，利用 SVD 分解提取聚合后的主成分来重构 LoRA 模块，并将残差分量合并回主干网络以确保结构表达力。
result: 在多项任务上的实验表明，FedMomentum 在收敛速度和最终准确率方面均显著优于现有的最先进方法。
conclusion: FedMomentum 通过保持训练动量和结构完整性，为大语言模型的高效联邦微调提供了一种更稳健且高性能的聚合方案。
---

...
tags: ["query:dml-dp"]
score: 6.0
evidence: 1比特通信限制下的均值估计，使用随机阈值查询
tldr: 本研究探讨了在严格1比特通信限制下的均值估计问题。作者提出了一种基于随机阈值查询的新型自适应估计器，适用于具有有界均值和k阶中心矩的各种分布。该方法在所有尾部机制下均达到了阶优样本复杂度，并证明了在有限方差情况下1比特量化存在不可避免的对数惩罚。研究还揭示了自适应策略相比非自适应策略在样本效率上的巨大优势，并提供了处理未知参数的算法变体。
selection_source: fresh_fetch
motivation: 旨在解决在极低通信带宽限制下，如何针对具有不同尾部特性的分布实现最优的均值估计。
method: 提出一种基于随机化阈值查询的自适应估计器，通过顺序选择阈值并根据1比特反馈逐步缩小均值范围。
result: 该估计器在所有k阶矩条件下均达到阶优复杂度，并证明了k=2时存在的量化惩罚是信息论意义上的基本限制。
conclusion: 自适应策略在1比特均值估计中至关重要，能将样本复杂度从对搜索空间的线性依赖降低到对数级依赖。
---

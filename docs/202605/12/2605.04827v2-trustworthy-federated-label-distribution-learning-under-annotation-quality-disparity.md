...
score: 7.0
evidence: 可信联邦标签分布学习
tldr: 针对联邦标签分布学习（Fed-LDL）中各客户端标注质量不一导致的聚合失效问题，本文提出FedQual框架。该框架通过全局语义锚点引导的质量自适应训练来校准低质量客户端，并利用基于可靠信息的服务器聚合机制重新分配权重。通过在四个新构建的基准数据集上的实验和理论证明，FedQual有效提升了异构标注质量下的模型性能。
selection_source: fresh_fetch
motivation: 联邦学习环境下各参与方标注质量存在显著差异，导致传统的基于样本量的聚合方法在处理标签分布学习时不再可靠。
method: 提出FedQual框架，结合了基于全局语义锚点的客户端自适应校准训练和基于有效可靠信息的服务器端加权聚合机制。
result: 在四个新构建的Fed-LDL基准数据集上的实验表明，FedQual显著优于现有方法，且理论上证明了其校准策略的有效性。
conclusion: FedQual通过质量感知机制成功解决了联邦标签分布学习中的信任困境，为处理异构标注质量提供了稳健的解决方案。
---

## 摘要
标签分布学习 (LDL) 将监督建模为实例级的概率分布，从而能够在固有歧义下进行细粒度学习，但其成功依赖于高保真度的标签分布，而这些分布获取成本高昂且通常包含噪声。受隐私敏感应用的启发，我们研究了联邦标签分布学习 (Fed-LDL)，其中数据孤岛进一步导致了客户端之间异构的标注质量，使得局部更新的可靠性不一，并破坏了基于样本量的聚合机制（如 FedAvg）。为了解决这一信任困境，我们提出了 FedQual，这是一个质量感知的 Fed-LDL 框架，包含两个耦合机制：(i) 由全局语义锚点引导的质量自适应客户端训练，在校准低质量客户端的同时保留高质量客户端的自主性；(ii) 可靠性感知的服务器聚合，通过有效的可靠信息而非原始样本量来重新加权客户端贡献。为了进行严格评估，我们构建了四个具有受控标注质量差异的新 Fed-LDL 基准（FER-LDL、FI-LDL、PIPAL-LDL 和 KADID-LDL）。我们进一步提供了理论保证，证明在异构监督质量下，针对特定客户端的校准严格优于任何统一校准。在所提基准上的广泛实验证明了 FedQual 的有效性。

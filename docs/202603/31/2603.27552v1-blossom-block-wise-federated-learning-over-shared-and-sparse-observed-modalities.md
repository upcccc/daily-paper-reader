...
## 摘要
多模态联邦学习（FL）对于自动驾驶系统和医疗保健等现实应用至关重要，在这些应用中，数据分布在具有不同且通常缺失模态的异构客户端上。然而，大多数现有的联邦学习方法假设模态可用性是统一的，这限制了它们在实践中的适用性。我们引入了 BLOSSOM，这是一个与任务无关的多模态联邦学习框架，旨在共享且稀疏观测的模态条件下运行。BLOSSOM 支持具有任意模态子集的客户端，并实现了模型组件的灵活共享。为了应对客户端和任务的异构性，我们提出了一种分块聚合策略，该策略选择性地聚合共享组件，同时保持任务特定块的私有性，从而实现部分个性化。我们在多个不同的多模态数据集上评估了 BLOSSOM，并分析了缺失模态和个性化的影响。我们的结果表明，分块个性化显著提高了性能，特别是在模态严重稀疏的情况下。在模态不完整的情况下，BLOSSOM 相比全模型聚合实现了 18.7% 的平均性能提升，而在模态互斥的情况下，提升幅度增加到 37.7%，突显了分块学习对于实际多模态联邦学习系统的重要性。
